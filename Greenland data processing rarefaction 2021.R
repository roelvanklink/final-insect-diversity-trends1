...
random_matrix <- rbind(random_matrix, random_matrix1)    
  ...
  # append this to other years 
    ...
    quart<- subset(alpha, Unit_in_data == "logNrQ1" |Unit_in_data == "logNrQ2" |Unit_in_data == "logNrQ3" |Unit_in_data == "logNrQ4" |Unit_in_data == "Singletons" )
    all.quart<- rbind(all.quart, quart)
...
dim(all.metrics)
saveRDS(all.metrics, file = "C:/Dropbox/Insect Biomass Trends/csvs/GreenlandAllValues 20210804.rds")
...
means<- aggregate(list(Number = all.metrics$Number), by = list(Plot_name = all.metrics$Plot_name, Year = all.metrics$Year, Unit_in_data = all.metrics$Unit_in_data), FUN = "mean")
...
plots<-read.csv( file = "c:\\Dropbox\\Insect Biomass Trends\\csvs/PlotData 4.1.csv"); dim(plots)
...
write.csv(greenlandTest, file = "C:\\Dropbox\\Insect Biomass Trends/csvs/greenlandRarefied 20210804.csv")
