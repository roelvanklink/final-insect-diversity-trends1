...
load("C:\\Dropbox\\Insect Biomass Trends/csvs/completeData2021pure.Rdata"); dim(completeData2021pure) # 18.8.20 66249 lines,  1676 plots
...
Biotime <- read.csv( file = "C:\\Dropbox\\Insect Biomass Trends/csvs/BioTIMEstd2021.csv"); unique(Biotime$Datasource_name)
...
data<- cc
data<- uk
quartiles<- quantile(log10(data$Number+1))


plot(density(log10(data$Number+1)), ylab = "")
hist(log10(data$Number+1), xlab = "log10 (Abundance)", main = "")
...
abline(v=max(log10(data$Number+1)), col="blue")
abline(v=max(log10(data$Number+1)*0.8), col="blue")
abline(v=max(log10(data$Number+1)*0.6), col="blue")
abline(v=max(log10(data$Number+1)*0.4), col="blue")
abline(v=max(log10(data$Number+1)*0.2), col="blue")
...
# Fig S2 influence of short and long timeseries #######
...
metadata_per_plotZ<- completeData2021pure %>% 
	group_by(Plot_ID) %>%
	summarise(
...
completeData2021pure<-   merge(completeData2021pure, metadata_per_plotZ )

completeData2021pureShort <-subset(completeData2021pure, Year> cutoffYear); dim(completeData2021pureShort)
# 
metadata_per_plotZ1<- completeData2021pureShort %>% 
	group_by(Plot_ID) %>%
	summarise(
		Plot_ID = unique(Plot_ID),
		Datasource_ID = unique(Datasource_ID),
...
metadata_per_plotZ1<- subset(completeData2021pureShort, !is.na(Number)) %>% 
	group_by(Plot_ID) %>%
	summarise(
		Plot_ID = unique(Plot_ID),
		Datasource_ID = unique(Datasource_ID),
...
completeData2021Short <-subset(completeData2021pure, Plot_ID %in% metadata_per_plotZ2$Plot_ID)
saveRDS(completeData2021Short, file = "completeData2021short.rds")
...
completeData2021long <-subset(completeData2021pure, Plot_ID %in% metadata_per_plotZ3$Plot_ID); dim(completeData2021long)
saveRDS(completeData2021long, file = "completeData2021long.rds")

# Figure short & long####
# alldata
inlaRichSum<- as.data.frame(readRDS("inlaRichnessSUMMARY.rds"))
richMarg<- readRDS("inlaRichnessMarginal.rds")
richMarg$Metric<- "Richness"
richMarg$Data<- "All" 

inlaAbSum<- as.data.frame(readRDS("inlaAbunSUMMARY.rds"))
abMarg<- readRDS("inlaAbunMarginal.rds" )
abMarg$Metric <- "Abundance"
abMarg$Data<- "All"

inlapieSum<- as.data.frame(readRDS("inlaENSPIESUMMARY.rds"))
pieMarg<- readRDS("inlaENSPIEMarginal.rds" )
pieMarg$Metric<- "ENS-PIE"
pieMarg$Data<- "All"

inlaRichLongSum<- as.data.frame(readRDS("sensitivity analysis/inlaRichLongSUMMARY.rds"))
inlaRichLongMarg<- readRDS("sensitivity analysis/inlaRichLongMarginal.rds" )
inlaRichLongMarg$Metric<- "Richness"
inlaRichLongMarg$Data<- "20 years"

inlaAbLongSum<- as.data.frame(readRDS("sensitivity analysis/inlaAbundLongSUMMARY.rds"))
inlaAbLongMarg<- readRDS("sensitivity analysis/inlaAbundLongMarginal.rds" )
inlaAbLongMarg$Metric<- "Abundance"
inlaAbLongMarg$Data<- "20 years"

inlaPieLongSum<- as.data.frame(readRDS("sensitivity analysis/inlaENSPIELongSUMMARY.rds"))
inlaPieLongMarg<- readRDS("sensitivity analysis/inlaENSPIELongMarginal.rds" )
inlaPieLongMarg$Metric<- "ENS-PIE"
inlaPieLongMarg$Data<- "20 years"

inlaRichShortSum<- as.data.frame(readRDS("sensitivity analysis/inlaRichShortSUMMARY.rds"))
inlaRichShortMarg<- readRDS("sensitivity analysis/inlaRichShortMarginal.rds" )
inlaRichShortMarg$Metric<- "Richness"
inlaRichShortMarg$Data<- "10 years"

inlaAbShortSum<- as.data.frame(readRDS("sensitivity analysis/inlaAbundShortSUMMARY.rds"))
inlaAbShortMarg<- readRDS("sensitivity analysis/inlaAbundShortMarginal.rds" )
inlaAbShortMarg$Metric<- "Abundance"
inlaAbShortMarg$Data<- "10 years"

inlaPieShortSum<- as.data.frame(readRDS("sensitivity analysis/inlaENSPIEShortSUMMARY.rds"))
inlaPieShortMarg<- readRDS("sensitivity analysis/inlaENSPIEShortMarginal.rds" )
inlaPieShortMarg$Metric<- "ENS-PIE"
...
univar$Metric<- factor(univar$Metric, levels = c ("Abundance", "Richness", "ENS-PIE" ))
...
#  Fig S3 effect of large datasets (large number of plots):  #####


metadata_per_dataset<-  completeData2021pure %>% 
 ...
 for ( q in c(1:3)){
		...
		completeData2021Good<- subset(completeData2021, !Datasource_ID %in% badDatasets$Datasource_ID) 
		completeData2021bad<- subset(completeData2021, Datasource_ID %in% badDatasets$Datasource_ID) 
		
		badDatasets<- badDatasets$Datasource_ID 
		badDatasets<- badDatasets[badDatasets %in% completeData2021$Datasource_ID]
		...
		dat1<- subset(completeData2021bad, Datasource_ID == dataset)
...
#==================================================
	...
	#===============================================================				
				 ...
				 if (nLocs > maxNrPlots){
						...
						dat1<- merge(dat1, plots)
							...
							max(dat1$Longitude) - min (dat1$Longitude)			
							max(dat1$Latitude) - min (dat1$Latitude)			
							
							# assume major axis of variation is latitude
							...
							dat1%>% group_by(Group) %>%
								summarise(
									Plot_ID = length(unique(Plot_ID)))
							
							# or each group get an overview of the quality of the plots 
												selectedPlots<- NULL
			...
			# or rather geographically equally sized areas) # doesnt work, because some areas have no samples (e.g. lapland)
											smpPltsPerDataset<- c(smpPltsPerDataset, smpPlot)
	...
	# close loop ====================================================================											
			...
			completeData2021badselected<- subset(completeData2021bad, Plot_ID %in% selectedPlots) 
			
			#stitch back together and assign name
				cDfixed<- rbind(completeData2021Good, completeData2021badselected) 
				path<- "C:\\Dropbox\\Insect Biomass Trends/csvs/"
				dfname<- paste0("completeData2021max", maxNrPlots, "plots"); dfname
				...
				dim(completeData2021max10plots) #works

			
#	and run models: Abundance, richness, pie. 
	...
	inlaRich50Sum<- as.data.frame(readRDS("sensitivity analysis/inlaRichness50plotsSUMMARY.rds"))
	inlaRich50Marg<- readRDS("sensitivity analysis/inlaRichness50plotsMarginal.rds" )
	inlaRich50Marg$Metric<- "Richness"
	inlaRich50Marg$Data<- "Max 50 plots per dataset"
	
	inlaAb50Sum<- as.data.frame(readRDS("sensitivity analysis/inlaAbundance50plotsSUMMARY.rds"))
	inlaAb50Marg<- readRDS("sensitivity analysis/inlaAbundance50plotsMarginal.rds" )
	inlaAb50Marg$Metric<- "Abundance"
	inlaAb50Marg$Data<- "Max 50 plots per dataset"
	
	inlaPIE50Sum<- as.data.frame(readRDS("sensitivity analysis/inlaENSPIE50plotsSUMMARY.rds"))
	inlaPIE50Marg<- readRDS("sensitivity analysis/inlaENSPIE50plotsMarginal.rds" )
	inlaPIE50Marg$Metric<- "ENS-PIE"
	inlaPIE50Marg$Data<- "Max 50 plots per dataset"
	
	inlaRich20Sum<- as.data.frame(readRDS("sensitivity analysis/inlaRichness20plotsSUMMARY.rds"))
	inlaRich20Marg<- readRDS("sensitivity analysis/inlaRichness20plotsMarginal.rds" )
	inlaRich20Marg$Metric<- "Richness"
	inlaRich20Marg$Data<- "Max 20 plots per dataset"
	
	inlaAb20Sum<- as.data.frame(readRDS("sensitivity analysis/inlaAbundance20plotsSUMMARY.rds"))
	inlaAb20Marg<- readRDS("sensitivity analysis/inlaAbundance20plotsMarginal.rds" )
	inlaAb20Marg$Metric<- "Abundance"
	inlaAb20Marg$Data<- "Max 20 plots per dataset"
	
	inlaPIE20Sum<- as.data.frame(readRDS("sensitivity analysis/inlaENSPIE20plotsSUMMARY.rds"))
	inlaPIE20Marg<- readRDS("sensitivity analysis/inlaENSPIE20plotsMarginal.rds" )
	inlaPIE20Marg$Metric<- "ENS-PIE"
	inlaPIE20Marg$Data<- "Max 20 plots per dataset"
	
	inlaRich10Sum<- as.data.frame(readRDS("sensitivity analysis/inlaRichness10plotsSUMMARY.rds"))
	inlaRich10Marg<- readRDS("sensitivity analysis/inlaRichness10plotsMarginal.rds" )
	inlaRich10Marg$Metric<- "Richness"
	inlaRich10Marg$Data<- "Max 10 plots per dataset"
	
	inlaAb10Sum<- as.data.frame(readRDS("sensitivity analysis/inlaAbundance10plotsSUMMARY.rds"))
	inlaAb10Marg<- readRDS("sensitivity analysis/inlaAbundance10plotsMarginal.rds" )
	inlaAb10Marg$Metric<- "Abundance"
	inlaAb10Marg$Data<- "Max 10 plots per dataset"
	
	inlaPIE10Sum<- as.data.frame(readRDS("sensitivity analysis/inlaENSPIE10plotsSUMMARY.rds"))
	inlaPIE10Marg<- readRDS("sensitivity analysis/inlaENSPIE10plotsMarginal.rds" )
	inlaPIE10Marg$Metric<- "ENS-PIE"
	...
	univarPlts$Metric<- factor(univarPlts$Metric, levels = c ("Abundance", "Richness", "ENS-PIE" ))
...
sads<- subset(completeData2021pure, Unit ==  "logNrQ1" |  Unit ==   "logNrQ2"|  Unit == "logNrQ3" |  Unit == "logNrQ4")
...
quart1<- as.data.frame(readRDS("quartile1SUMMARY.rds"))
q1Marg<- readRDS("quartile1Marginal.rds" )
q1Marg$Metric<- "Quartile 1"

quart2<- as.data.frame(readRDS("quartile2SUMMARY.rds"))
q2Marg<- readRDS("quartile2Marginal.rds" )
q2Marg$Metric<- "Quartile 2"

quart3<- as.data.frame(readRDS("quartile3SUMMARY.rds"))
q3Marg<- readRDS("quartile3Marginal.rds" )
q3Marg$Metric<- "Quartile 3"

quart4<- as.data.frame(readRDS("quartile4SUMMARY.rds"))
q4Marg<- readRDS("quartile4Marginal.rds" )
...
ggsave(filename = "Fig S4 quartiles.png" , path = figure_path, width = 12, height = 12,  units = "cm",dpi = 300, device = "png")
ggsave(filename = "Fig S4 quartiles.pdf" , path = figure_path, width = 12, height = 12,  units = "cm",dpi = 300, device = "pdf")
				...
				# Effect of first year for beta diversity:  #####
				# add 1-last year as input to array job 
				...
				origBetaMeans<- data.frame(Realm = rep(c("Freshwater", "Terrestrial"),4), 
																	 Metric = rep(c("Jaccard", "ES Jaccard", "Morisita-Horn", "ES Morisita-Horn"), each = 2 ), 
																	 Mean = 	c(inlaJacSum$mean[3:4], inlaJacDifSum$mean[3:4], inlaHornSum$mean[3:4], inlaHornDifSum$mean[3:4]))
				origBetaMeans$Metric<- ordered(origBetaMeans$Metric, 
																			 levels = (c("Jaccard", "ES Jaccard", "Morisita-Horn", "ES Morisita-Horn" )))
				
				# model outputs based on 1- last year 
				
				inlaJacLastSum<- readRDS("sensitivity analysis/inlaJaccardLastSUMMARY.rds")
				jacLastMarg<- readRDS("sensitivity analysis/inlaJaccardLastMarginal.rds" )
				jacLastMarg$Metric<- "Jaccard"
				
				inlaJacDifLastSum<- (readRDS("sensitivity analysis/inlaJacDifLastSUMMARY.rds"))
				jacDifLastMarg<- readRDS("sensitivity analysis/inlaJacDifLastMarginal.rds" )
				jacDifLastMarg$Metric<- "ES Jaccard"
				
				inlaHornLastSum<- as.data.frame(readRDS("sensitivity analysis/inlaHornLastSUMMARY.rds"))
				hornLastMarg<- readRDS("sensitivity analysis/inlaHornLastMarginal.rds" )
				hornLastMarg$Metric<- "Morisita-Horn"
				
				inlaHornDifLastSum<- as.data.frame(readRDS("sensitivity analysis/inlaHornLastDifSUMMARY.rds"))
				hornDifLastMarg<- readRDS("sensitivity analysis/inlaHornLastDifMarginal.rds" )
				hornDifLastMarg$Metric<- "ES Morisita-Horn"
				
				origBetaMeans$MeanLast<- 	c(inlaJacLastSum$mean[3:4], inlaJacDifLastSum$mean[3:4], inlaHornLastSum$mean[3:4], inlaHornDifLastSum$mean[3:4])
				betaLastMarg<- rbind(jacLastMarg, jacDifLastMarg, hornLastMarg, hornDifLastMarg)
				betaLastMarg$Metric<- ordered(betaLastMarg$Metric, 
																			levels = (c("Jaccard", "ES Jaccard", "Morisita-Horn", "ES Morisita-Horn" )))
				
				mean <- betaLastMarg %>%
					group_by(Realm, Metric) %>%
					filter(y == max(y, na.rm=TRUE))
				...
				ggplot(betaLastMarg, aes(x = -x, y = y))+
					geom_area(  aes(x = -x, y = y80, fill = Realm), alpha = 0.8)+
					geom_area(  aes(x = -x, y = y90, fill = Realm), alpha = 0.6)+
					geom_area(  aes(x = -x, y = y95, fill = Realm), alpha = 0.3)+
					geom_area( aes(x = -x, y = y, fill = Realm), alpha = 0.3)+
					geom_vline(data= mean, aes(xintercept = -x, linetype=Realm))+
					geom_vline(data = origBetaMeans, aes(xintercept = Mean, linetype=Realm) )+
					#geom_vline(xintercept = 0)+
				...
				# are the first year and last year beta correlated? 
				testdat<- subset(completeData2021, Unit == "Jaccard" | Unit == "JaccardLast")
				testdat<- subset(testdat, !is.na(Number)); dim(testdat)
				testdat<- subset(testdat, Number>0); dim(testdat)
				
				testdat<- dcast(testdat, 
												Realm + Datasource_ID +Plot_ID + Year~ Unit , value.var =  "Number") ; dim(testdat)
				length(unique(testdat$Plot_ID))
				cor(testdat$Jaccard, testdat$JaccardLast , use = "complete.obs")
				plot(subset(testdat, Realm == "Terrestrial")$Jaccard, subset(testdat, Realm == "Terrestrial")$JaccardLast, col = "chocolate4", pch = 16)
				points(subset(testdat, Realm == "Freshwater")$Jaccard, subset(testdat, Realm == "Freshwater")$JaccardLast, col = "blue", pch = 16)
				points(testdat$Jaccard, testdat$JaccardLast)
				
				
				testmeta<- testdat %>% group_by(Plot_ID) %>%
					summarize(nyr = length(Year)); dim(testmeta)
				testmeta2<- subset(testmeta, nyr > 2); dim(testmeta2)
				testdat<- subset(testdat, Plot_ID %in% testmeta2$Plot_ID); dim(testdat)
				
				smp<- sample(unique(testdat$Plot_ID), 100)
				
				ggplot(subset(testdat, Plot_ID %in% smp), aes(x = Jaccard, y = JaccardLast, color = Realm))+
					geom_point()+
					facet_wrap(.~Plot_ID)
 ...
 # Fig S5 effect of bias towards Europe and North America #####
			
			metadata_per_datasetX<- subset(completeData2021pure, Unit == "richness") %>% 
				group_by(Datasource_ID) %>%
				summarise(
					Datasource_ID = unique(Datasource_ID),
					Datasource_name = unique(Datasource_name), 
...
median(subset(datasets, Var2 == "Freshwater")$Freq )
 ...
 # combine easter Asia and Australia,  discard Africa 

head(completeData2021pure)
...
completeData2021$Continent2<- completeData2021$Continent
completeData2021$Continent2[completeData2021$Continent2 == "Oceania"] <- "Rest"
completeData2021$Continent2[completeData2021$Continent2 == "Asia"] <- "Rest"
completeData2021$Continent2[completeData2021$Continent2 == "Latin America"] <- "Rest"
completeData2021$Continent2[completeData2021$Continent2 == "Africa"] <- "Rest"
# sorry people
metadata_per_cont<- completeData2021 %>% 
...
metadata_per_cont$Metric[metadata_per_cont$Unit == "ENSPIE"]<- "ENS-PIE"
...
metadata_per_cont$Metric<- factor(metadata_per_cont$Metric, levels = c ("Abundance", "Richness", "ENS-PIE" ))
metadata_per_cont$x<- NA
metadata_per_cont$x[metadata_per_cont$Metric == "ENS-PIE" & metadata_per_cont$Realm == "Terrestrial"] <- -0.0015
metadata_per_cont$x[metadata_per_cont$Metric == "ENS-PIE" & metadata_per_cont$Realm == "Freshwater"] <- 0.008
...
# not used: 
contPIESum<- as.data.frame(readRDS("sensitivity analysis/inlaENSPIEFixContSUMMARY.rds"))
contAbSum<- as.data.frame(readRDS("sensitivity analysis/inlaAbundFixContSUMMARY.rds"))
contRichSum<- as.data.frame(readRDS("sensitivity analysis/inlaRichFixContSUMMARY.rds"))

# used
cont2PIESum<- as.data.frame(readRDS("sensitivity analysis/inlaENSPIEFixCont2SUMMARY.rds"))
cont2AbSum<- as.data.frame(readRDS("sensitivity analysis/inlaAbundFixCont2SUMMARY.rds"))
cont2RichSum<- as.data.frame(readRDS("sensitivity analysis/inlaRichFixCont2SUMMARY.rds"))
cont2PIEmarg<- (readRDS("sensitivity analysis/inlaENSPIEFixCont2MARG.rds"))
cont2Abmarg<-   (readRDS("sensitivity analysis/inlaAbundFixCont2MARG.rds"))
cont2Richmarg<-   (readRDS("sensitivity analysis/inlaRichFixCont2MARG.rds"))
...
metrics <- c("Abundance", "Richness", "ENS-PIE" )	
...
dim(ContMargs)	
table(ContMargs$Continent, ContMargs$Metric, ContMargs$Realm)
	
ContMargs$y[ContMargs$y < 1 ]<- NA	
	
	
ContMargs$Metric<- factor(ContMargs$Metric, levels = c ("Abundance", "Richness", "ENS-PIE" ))
ContMargs$Continent<- factor(ContMargs$Continent, levels = c ("Europe", "North America" , "Rest" ))
	...
	ggsave(filename = "Fig S5 Geography.png" , path = figure_path, width = 18, height = 12,  units = "cm",dpi = 300, device = "png")
	ggsave(filename = "Fig S5 Geography.pdf" , path = figure_path, width = 18, height = 12,  units = "cm",dpi = 300, device = "pdf")
